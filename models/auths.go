...
type Auth struct {
	...
	Auth      null.String `boil:"auth" json:"auth,omitempty" toml:"auth" yaml:"auth,omitempty"`
...
var (
	authColumns               = []string{"id", "created_at", "updated_at", "deleted_at", "auth"}
	authColumnsWithoutDefault = []string{"created_at", "updated_at", "deleted_at", "auth"}
	authColumnsWithDefault    = []string{"id"}
...
type (
	...
	AuthHook func(boil.Executor, *Auth) error
...
// OneP returns a single auth record from the query, and panics on error.
func (q authQuery) OneP() *Auth {
	o, err := q.One()
	if err != nil {
...
func (q authQuery) One() (*Auth, error) {
	o := &Auth{}
...
// AllP returns all Auth records from the query, and panics on error.
...
func (q authQuery) All() (AuthSlice, error) {
	var o []*Auth
...
// CountP returns the count of all Auth records in the query, and panics on error.
...
func (q authQuery) Exists() (bool, error) {
	var count int64
...
// AuthsG retrieves all records.
func AuthsG(mods ...qm.QueryMod) authQuery {
	return Auths(boil.GetDB(), mods...)
}

// Auths retrieves all the records using an executor.
func Auths(exec boil.Executor, mods ...qm.QueryMod) authQuery {
	mods = append(mods, qm.From("\"auths\""))
	return authQuery{NewQuery(exec, mods...)}
}

// FindAuthG retrieves a single record by ID.
func FindAuthG(id int, selectCols ...string) (*Auth, error) {
	return FindAuth(boil.GetDB(), id, selectCols...)
}

// FindAuthGP retrieves a single record by ID, and panics on error.
func FindAuthGP(id int, selectCols ...string) *Auth {
	retobj, err := FindAuth(boil.GetDB(), id, selectCols...)
	if err != nil {
		panic(boil.WrapErr(err))
	}
...
// FindAuthP retrieves a single record by ID with an executor, and panics on error.
...
// UpdateG a single Auth record. See Update for
// whitelist behavior description.
func (o *Auth) UpdateG(whitelist ...string) error {
	return o.Update(boil.GetDB(), whitelist...)
}

// UpdateGP a single Auth record.
...
// UpdateP uses an executor to update the Auth, and panics on error.
...
func (q authQuery) UpdateAll(cols M) error {
	queries.SetUpdate(q.Query, cols)

	_, err := q.Query.Exec()
	if err != nil {
		return errors.Wrap(err, "models: unable to update all for auths")
	}
...
func (o AuthSlice) UpdateAll(exec boil.Executor, cols M) error {
	ln := int64(len(o))
	if ln == 0 {
		return nil
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to update all in auth slice")
	}
...
// DeleteP deletes a single Auth record with an executor.
...
// DeleteG deletes a single Auth record.
...
// DeleteGP deletes a single Auth record.
...
// and overwrites the original object slice with the newly updated slice.
func (o *AuthSlice) ReloadAllG() error {
	if o == nil {
		return errors.New("models: empty AuthSlice provided for reload all")
	}
...
func AuthExists(exec boil.Executor, id int) (bool, error) {
	var exists bool
...
// AuthExistsG checks if the Auth row exists.
func AuthExistsG(id int) (bool, error) {
	return AuthExists(boil.GetDB(), id)
}

// AuthExistsGP checks if the Auth row exists. Panics on error.
func AuthExistsGP(id int) bool {
	e, err := AuthExists(boil.GetDB(), id)
	if err != nil {
		panic(boil.WrapErr(err))
	}
...
// AuthExistsP checks if the Auth row exists. Panics on error.
func AuthExistsP(exec boil.Executor, id int) bool {
	e, err := AuthExists(exec, id)
	if err != nil {
		panic(boil.WrapErr(err))
	}
